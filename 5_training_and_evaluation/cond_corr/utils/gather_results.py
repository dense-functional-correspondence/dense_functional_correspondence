...
baselines = ["Chance", "Chance-CogVLM", "Chance-ManipVQA-Part", "Chance-ManipVQA-Action", "Dino", "SD", "SD-Dino", "Dino-CogVLM", "Spatial_fullImg_bgAug-CogVLM", "Dino-ManipVQA-Part", "Spatial_fullImg_bgAug-ManipVQA-Part", "Dino-ManipVQA-Action", "Spatial_fullImg_bgAug-ManipVQA-Action"]
experiments = baselines + experiments
annotation_dir = "<#TODO: path to your Objaverse selected_annotations_seen.json directory>"
...
for experiment in experiments:
    ...
    epochs = [x for x in epochs if x == "objaverse_test_seen" or "objaverse_test_seen_epoch_" in x]
    epochs.sort()
        ...
        if os.path.exists(os.path.join(result_dir, suffix)):
            ...
            for action, entries in annotations.items():
                for obj_pair, trials in entries.items():
                    ...
                    obj1_category = obj1.split("---")[-1].replace(".glb", "")
                    obj2_category = obj2.split("---")[-1].replace(".glb", "")
...
df.to_csv('./objaverse_aggregated_results_seen_test.csv', index=False)
