...
def main(category, ports, img_dir, out_dir, obj2part, n_trials):
    ...
    os.makedirs(os.path.join(out_dir, "part_annotations"), exist_ok=True)
    os.makedirs(os.path.join(out_dir, "part_viz_cogvlm"), exist_ok=True)

    for part, description in part2description.items():
        print(f"\nFinding masks for {part} of {category}.\nUsing the following descriptions:")
        out_dct = {}
        out_dct_f = os.path.join(out_dir, "part_annotations/"+part.replace(" ", "_")+".json")
        ...
        for itr, image_path in enumerate(image_files):
            ...
            img_num = int(img_name.removesuffix('.png'))

            orig_img = Image.open(image_path).convert("RGB")
            encoded_img = encode_image(square_pad(orig_img))
            viz_fn = f"part_viz_cogvlm/{remove_image_extension(img_name)}-{part.replace(' ', '_')}-"
...
def run_main(args, categories, ports, obj2part, cluster_id):
    ...
    for category in categories:
        ...
        img_dir = os.path.join(args.root_dir, category, "rgb_images")
...
if __name__ == "__main__":
    ...
    categories = [x for x in categories if os.path.exists(os.path.join(args.root_dir, x, "rgb_images"))]
    print(f"There are {len(categories)} complete categories.")
