...
from cogvlm_scripts import constants
from accelerate import init_empty_weights, infer_auto_device_map, load_checkpoint_and_dispatch


TOKENIZER_PATH = constants.TOKENIZER_PATH
GROUNDING_MODEL_PATH = constants.GROUNDING_MODEL_PATH
CHAT_MODEL_PATH = constants.CHAT_MODEL_PATH
IP_PORT_FILE = "<#TODO: path to a .txt file that stores ip and port information>"
...
if os.environ.get('QUANT_ENABLED'):
    ...
    QUANT_ENABLED = False
print(f"Quant Enabled is {QUANT_ENABLED}.")
...
def get_ip_address():
    ...
    if result.returncode != 0:
        print(f"Error running ifconfig: {result.stderr}")
        return None
    ...
    # Filter out loopback address and return the first non-loopback address
...
if __name__ == "__main__":
    ...
    parser.add_argument('port', help='what port to use')
    # parser.add_argument('--gpu_num', type=int, help='what gpu is being used, two consecutive ones, take the first.')
    parser.add_argument('--chat', action='store_true', help='use the chat model')
    ...
    if 'cuda' in DEVICE:
        if QUANT_ENABLED:
            ...
            device_map = infer_auto_device_map(model, max_memory={0:'20GiB', 1:'20GiB'}, no_split_module_classes=['CogVLMDecoderLayer', 'TransformerLayer'])
    ...
    if not os.path.exists(IP_PORT_FILE):
        with open(IP_PORT_FILE, 'w') as file:
            pass  # Create an empty txt file
    ip_address = get_ip_address()
    with open(IP_PORT_FILE, 'a') as file:
        file.write(f"{ip_address} {args.port}\n")
